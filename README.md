...
- **Units and control** — `perm`, `if`, `node`
- **Called after `::`** — `order`, `oneof`, `icase`, `is`, `not`, `subkind`,
  `part`, `char_count`, `to_num`, `iter_steps`, `until`, `first`, `begin`,
  `end`, `clear`
...
body means the action is written in C. `^` between a counter and a unit is the
**Less** glue — the counter stops early so the unit after it can match. All
three are highlighted as operators rather than left plain, because a reader
scanning a grammar needs to see them.
